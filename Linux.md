...
## 五、Linux、Nginx、Tomcat的关系

### 在标准的生产环境（企业级部署）中，它们三者是这样完美合体的：

1. **底层地基**：你在服务器上装了 Linux 系统，提供硬件运行环境。
2. **前台接待**：客人的请求过来了，装在 Linux 上的 Nginx 最先收到请求。如果是要看前端网页或图片，Nginx 直接把文件丢给客人；如果是要登录、查数据（动态请求），Nginx 转发给后面的 Tomcat。
3. **后台做饭**：Spring Boot 项目（内嵌 Tomcat） 收到 Nginx 转发的请求，赶紧用 Java 计算好结果，再原路返回给 Nginx，Nginx 最终呈递给浏览器。